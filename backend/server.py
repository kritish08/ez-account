...
def _b64url_to_bytes(s: str) -> bytes:
    """Convert URL-safe base64 (from browser) to bytes."""
...
def _rebuild_attested_credentials(passkeys: list) -> list:
    ...
    Stored format: {id: base64-cred-id, public_key: base64-cbor-cose-key}.
    """
    from fido2.webauthn import AttestedCredentialData, CoseKey
    result = []
    for pk in passkeys:
        try:
...
async def register_passkey_begin(current_user: dict = Depends(get_current_user)):
    ...
    from fido2.webauthn import (
        ...
        PublicKeyCredentialType, UserVerificationRequirement, AuthenticatorAttachment,
    )
...
async def authenticate_passkey_complete(data: WebAuthnAuthenticateComplete):
    ...
    try:
        from fido2.webauthn import (
            ...
            AuthenticatorAssertionResponse, AuthenticationResponse,
        )
...
@api_router.delete("/auth/passkeys/{credential_id:path}")
...
@api_router.patch("/auth/passkeys/{credential_id:path}")
